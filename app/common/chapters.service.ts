...
export class ChapterService {
  ...
  getChapters(): Chapter[] {
   ...
   getIndexByChapterId(id: string): number {
    for (var i = 0; i < this.chapcodes.length; i++)
      if (this.chapcodes[i].id == id)
        return i;
  ...
  delete(chapter) {
    console.log(this.chapcodes);
    let ind: number = this.getIndexByChapterId(chapter.id);

    console.log("Deleting the book with index: ", ind);
    console.log(" and id: ", this.chapcodes[ind].id);
    if (ind > -1) {
      this.chapcodes.splice(ind, 1);
